...
SELECT * from product;
--1. Retrieve the ages of all customers from the USA

SELECT * from customers WHERE  country='USA' order BY age;

select product_name from product where product_name LIKE '%Samsung%';

select product_type from product where product_type LIKE '%Smartphone%';

CREATE procedure Producttype_smartphones
as
BEGIN
select product_type from product where product_type LIKE '%Smartphone%';
END;

EXEC Producttype_smartphones;
